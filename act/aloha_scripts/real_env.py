...
class RealEnv:
    ...
    def __init__(self, init_node, setup_robots=True, setup_base=False):
        ...
        self.puppet_bot_right = InterbotixManipulatorXS("vx300s", group_name="arm", gripper_name="gripper", robot_name=f'puppet_right', moving_time=2.0, accel_time=0.3, gripper_pressure=0.5, gripper_pressure_lower_limit=150, gripper_pressure_upper_limit=350, init_node=init_node)
        print('init over')
        if setup_robots:
            self.setup_robots()
        ...
        # self.setup_t265()
        print('real_env dxl start')
        self.setup_dxl()
        print('real_env recorder_right start')
        # self.recorder_left = Recorder('left', init_node=False)
        self.recorder_right = Recorder('right', init_node=False)
        print('real_env image_recorder image_recorder start')
        self.image_recorder = ImageRecorder(init_node=False)
        print('real_env gripper_command image_recorder start')
    ...
    def setup_dxl(self):
        ...
        self.dxl_client = DynamixelClient([1, 2], port='/dev/ttyDXL', lazy_connect=True)
    ...
    def get_qpos(self):
        ...
        left_arm_qpos = np.zeros(6)
        left_gripper_qpos = np.zeros(1)
    ...
    def _reset_joints(self):
        ...
        move_arms([self.puppet_bot_right], [reset_position, reset_position], move_time=1)

    def _reset_gripper(self):
        """Set to position mode and do position resets: first open then close. Then change back to PWM mode  move_grippers([self.puppet_bot_left, self.puppet_bot_right], [PUPPET_GRIPPER_JOINT_OPEN] * 2, move_time=0.5)
    ...
    def step(self, action, base_action=None, get_tracer_vel=False, get_obs=True):
        ...
        self.puppet_bot_right.arm.set_ee_pose_components(right_action[:6], blocking=False)
        #print(right_action[-1])
