...
def build_ACT_model_and_optimizer(args_override):
    ...
    for k, v in args_override.items():
        setattr(args, k, v)
    # build_ACT_model_and_optimizer中又通过调用了act - main / detr / models / _init_.py中的build_ACT_model(args)
    model = build_ACT_model(args)  # vit_encoder = ()
    model.cuda()
    ...
    # # 检查是否有重叠参数
    # all_params = set(part1_params) & set(part2_params)
    # if all_params:
    #     print("Warning: Overlapping parameters", all_params)
    param_dicts = [
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if "obs_encoder" not in n and p.requires_grad
            ]
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if "obs_encoder" in n and p.requires_grad
            ],
            "lr": args.lr_backbone,
        },
    ]
